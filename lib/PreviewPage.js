...
export class PreviewPage {
  ...
  async #registerPageCallback(functionName, callback) {
    // 同じタブに複数回同じコールバック関数を登録しようとするとエラーが起きるので存在チェックする
    if (
      await this.page.evaluate((target) => !(target in window), functionName)
    ) {
      this.page.exposeFunction(functionName, callback);
    }
  }
  ...
  async init({ selectedViewFileName }) {
    ...
    await this.#registerPageCallback(
    ...
    // ページ内でビューの検索が行われたときに呼ばれるコールバック関数を登録する
    await this.#registerPageCallback(
